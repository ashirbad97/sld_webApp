...
router.get("/level/:level/submodule/:submodule", auth, async (req, res) => {
  try {
    ...
    const subModuleName = await SubModule.findsubModuleName(
...
router.post("/addPatientFormData", async (req, res) => {
  try {
    ...
    await patient.save()
    credentials = await Patient.findOne({
      patientId: req.body.patientId,
    }).select("patientId password")
...
router.post("/", async (req, res) => {
  try {
    res.status(200)
    var patient = await Patient.authenticateuser(
      req.body.username,
      req.body.password
    )
